<html>
<head>
    <title>Opti Technology | Timeline</title>
<?php
	include 'Design/SectorConstants/header.php';
?>
<body>	
	<section class="content">
		<div class="head">
			<h1> Timeline </h1>
			<p> The expected projection for Opti </p>
		</div>
		
		<div class="timeline" month="September">
			<span class="month-pointer"></span>
			<p>The long-awaited Beta! This will be the first time Optimator is released publicly
			for you to play around with. The beta will be free for everyone as the software will still lack certain features and
			will have a lower visual quality, but it will allow you to test out the base functionality and get an idea of 
			how Optimator works.</p>
			<p>Subscribe to notifications to be informed as soon as it's ready!</p>
		</div>
		<div class="timeline" month="October">
			<span class="month-pointer"></span>
			<p>We'll be doing lots of work under the hood to prepare Optimator for its official launch!</p>
		</div>
		<div class="timeline" month="November">
			<span class="month-pointer"></span>
			<p>Keeping busy...</p>
		</div>
		<div class="timeline" month="December">
			<span class="month-pointer"></span>
			<p>The Optimator official release is planned for December- just in time for Christmas! This full version may not
			look too different to the beta, but will include access to all future upgrades. </p>
			<p>Subscribe to notifications to be informed as soon as it's ready!</p>
		</div>
		<div class="timeline" month="">
			<br/><br/>
		</div>
	</section>
	
	<?php
		include 'Design/SectorConstants/footer.php';
	?>
</body>